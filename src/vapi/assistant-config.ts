import type { AssistantResult } from "../assistant/service.js";

export const TRANSFER_MESSAGE =
  "I'm not sure about that. Let me transfer you to someone who can help.";

// Provisional HTTP contract: Atiye must confirm that all five statuses survive.
// These rules are used directly in the deployed prompt, not a detached mapping.
export const STATUS_RULES = {
  ...
  not_found: "Read text verbatim. Do NOT automatically transfer. If the caller insists or asks for something different, you may offer a human transfer; transfer only if they accept or explicitly request a human.",
...
} satisfies Record<AssistantResult["status"], string>;
...
Apply these five distinct status rules; never collapse them to a boolean:
...
For an explicit request for a human, invoke transferCall. Never transfer merely because clarification_required or not_found was returned.
...
export function createLookupTool(baseUrl: string) {
  ...
  return {
    ...
    description: "Required for every factual Zuki question. Read the returned text verbatim and follow the five status rules in the system prompt.",
    method: "POST",
